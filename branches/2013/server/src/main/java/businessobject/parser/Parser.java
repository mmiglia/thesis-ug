			...
			for (String s : o.getVerbs(l)){ // for every synonyms of the command
				...
				for (String r : regex){ // for every possible regex
					...
					if (text.contains(r)){
						...
						// check if exist a delimiter at the beginning of the string
						String str;
						...
						// se non è stato trovato alcun delimiter
						if (!find_delim) {
							for (int i=0; i<args.size(); i++) {
								// aggiungo all'inizio di ogni args il verbo che fa esso stesso da delimitatore
								str = args.get(i);
								str = r+" "+str;							
								args.set(i, str);
